...
public class CustomLineRecordReader extends RecordReader<LongWritable, Text> {
    ...
    private Text nextLine = new Text("");
    private static final Logger LOG = Logger.getLogger(CustomLineRecordReader.class);
    ...
    public boolean nextKeyValue() throws IOException {
        ...
        int newSize = 0;
        //variable to store a line that starts with "C"
        Text tmp = new Text("");
        ...
        while (pos < end) {
            ...
            if (newSize < maxLineLength) {
                // if value starts with "C" then append all lines until next "C" or EOF
                if (value.toString().startsWith("C") || nextLine.toString().startsWith("C")) {
                    in.readLine(tmp, maxLineLength,
                            Math.max((int) Math.min(
                                            Integer.MAX_VALUE, end - pos),
                                    maxLineLength));
                    ...
                    // this is true after the first time a "C" line occurs
                    if (nextLine.toString().startsWith("C")) {
                        Text swap = value;
                        value = new Text("");
                        value.append(nextLine.getBytes(), 0, nextLine.getLength());
                        value.append(EOL.getBytes(), 0, EOL.getLength());
                        value.append(swap.getBytes(), 0, swap.getLength());
                    }

                    //in case there is only only one v between two "C" lines
                    if (tmp.toString().startsWith("C")) {
                        nextLine = tmp;
                    }
                    // while new line NOT starts with "C"
                    // assumption here is that between "C" blocks we can find only lines starting with "V"
                    while (!tmp.toString().startsWith("C")) {
                        //if meet the end of file
                        if (tmp.toString().compareTo("") == 0) break;
                        value.append(EOL.getBytes(), 0, EOL.getLength());
                        ...
                        in.readLine(tmp, maxLineLength,
                                Math.max((int) Math.min(
                                                Integer.MAX_VALUE, end - pos),
                                        maxLineLength));
                        // store next line in class variable
                        nextLine = tmp;
                    }
                }
                break;
